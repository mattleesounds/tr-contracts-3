...
contract TRTest5 is ERC1155, Ownable, ReentrancyGuard, Pausable {
    ...
    function mintBatchSongs(
        ...
        for (uint256 i = 0; i < songIds.length; i++) {
            ...
            totalCost += costs[i] + (platformFee * quantities[i]); // Apply platformFee to each quantity
        }

        require(msg.value >= totalCost, "Insufficient payment");

        // Effects & Event Emission (Moved here for readability)
        for (uint256 i = 0; i < songIds.length; i++) {
            Song storage song = songs[songIds[i]];
            ...
            emit SongMinted(
                ...
                artistAddresses[i],
                quantities[i],
                costs[i] // Event emits artist payment only
            );
        }

        // Interactions
        for (uint256 i = 0; i < songIds.length; i++) {
            payable(artistAddresses[i]).transfer(costs[i]); // Transfer artist payment only
        }
    ...
    function pause() public onlyOwner {
        _pause();
        emit ContractPaused(owner()); // Event emission moved here
    }

    function unpause() public onlyOwner {
        _unpause();
        emit ContractUnpaused(owner()); // Event emission moved here
    }
    ...
    function withdrawPlatformFees() public onlyOwner nonReentrant {
        ...
        emit PlatformFeesWithdrawn(owner(), balance); // Event emission moved here
    }
